...
func NewDriver(subnets []string, endpoint types.SubmarinerEndpoint, cluster types.SubmarinerCluster) (cable.Driver, error) {
	var err error
	...
	if err = w.setWGLink(subnets, cluster); err != nil {
	...
	pub = priv.PublicKey()
	if endpoint.Spec.BackendConfig == nil {
		endpoint.Spec.BackendConfig = make(map[string]string)
	}
	endpoint.Spec.BackendConfig[PublicKey] = pub.String()
...
func (w *wireguard) setWGLink(subnets []string, cluster types.SubmarinerCluster) error {
	...
	ip, err := w.discoverInternalIP(subnets, cluster)
	if err != nil {
...
func (w *wireguard) discoverInternalIP(subnets []string, cluster types.SubmarinerCluster) (string, error) {
	cidrs := parseSubnets(subnets)
